...
export const questsRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  const callGemini = async (prompt: string, image?: { data: string, mimeType: string }, attempt: number = 0): Promise<string> => {
    ...
    try {
      ...
      const cavemanSystemPrompt = `[SYSTEM DIRECTIVE: OPERATE UNDER CAVEMAN PROTOCOL TO MINIMIZE TOKEN USAGE. ALL RESPONSES GENERATED BY THIS MODEL MUST BE EXTREMELY TERSE, SHORT, AND TO THE POINT. REMOVE ALL FILLER WORDS, REDUNDANT SENTENCE CONNECTORS, ARTICLES (A/AN/THE), AND GENERAL FORMALITY/GREETINGS/EXPLANATIONS. FOR EXAMPLES OR TEXT VALUES GENERATED, DRAFT THEM TO BE MAXIMALLY CONCISE AND DIRECT. DONT CHANGE ANY STRUCTURAL CODE FORMATS, AND DONT AFFECT ESSENTIAL FORMULAS/NUMERICAL PRECISION OR GIVEN FORMATTING TEMPLATES/JSON STRUCTURE SCHEMAS.]\n\n`;
