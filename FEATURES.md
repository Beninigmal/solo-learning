# 🔮 Collegium — Manual de Regras de Negócio e Sistemas Gamificados (Single Source of Truth)

Este documento é a **fonte única de verdade (Single Source of Truth)** do **Collegium**. Ele unifica e organiza todas as regras de negócio, mecânicas de RPG, especificações técnicas, regras de distribuição do Monarch Engine, o catálogo de artefatos e a estrutura SaaS.

---

## 🏛️ 1. Perfis de Acesso e Autenticação (RBAC & Segurança)

O Collegium é um sistema **multi-tenant** isolado por instituição acadêmica. O controle de acesso é baseado em perfis (Role-Based Access Control) acoplado a tokens JWT:

### A. Perfis de Usuário (Roles)
*   **👑 SUPERADMIN (Admin Geral)**: Gestão de escolas/tenants e cadastro inicial dos Arquitetos.
*   **🏛️ ARQUITETO (Coordenação Escolar)**: Cadastro de turmas, alunos, matérias, professores, alocação de vínculos e execução do gerador de grades horárias via sistema integrado e APIs atômicas.
*   **⚔️ MESTRE (Professor)**: Forja de rascunhos de quests por IA (Gemini / Nvidia Llama 3.1), gerenciamento de Mega Bosses, edição manual/transmutação de questões, monitoramento acadêmico e relatórios de risco.
*   **🧑‍🚀 CAÇADOR (Aluno/Player)**: Resolução de quests diárias comuns, combates em Party Raids contra Mega Bosses, inventário de artefatos e gestão de grupos.

### B. Fluxo de Primeiro Acesso & Biometria
*   **Acesso Provisório**: Novos usuários logam usando suas credenciais padrão (Senha provisória/Código da Party).
*   **Configuração Inicial**: São imediatamente forçados a cadastrar uma senha definitiva de 4 a 12 caracteres e escolher um Nickname (único dentro de sua instituição, formatado como `codigo@nickname`).
...
*   **Jurídico e LGPD**: O usuário deve assinar eletronicamente os Termos de Consentimento (`acceptedTermsAt`). Para caçadores menores de idade, exige-se o registro do nome do responsável legal (`parentConsentName`). O app bloqueia requisições e sons de notificação até que o termo seja aceito. Caso seja recusado, o logout é imediato.
...
*   A quest entra em uma fila de tolerância (TTL) com duração máxima de **40 minutos** (`WAIT_TTL_MINUTES = 40`). O aluno deve reabrir e responder dentro deste limite, ou a quest expira e cai no Baú como falha.
...
*   **Devolutiva Direcionada**: Durante o cooldown, a IA indica onde o raciocínio falhou (ex: *"Você montou a equação certa, mas errou a simplificação"*), sem entregar a resposta correta.
...
*   **Destino de Falhas**: Quests expiradas no Wait Mode ou erradas são enviadas ao **Baú de Quests Perdidas**.
*   **Penalidade de XP (Decaimento)**: Cada erro ou expiração impõe uma penalidade de **-25% de XP acumulado por erro** sobre aquela quest específica (`questXp * 0.75^erros`), respeitando o piso mínimo de **25 XP**.
*   **Recuperação**: O aluno pode tentar responder a questão novamente no Baú a qualquer momento (respeitando o cooldown). Se acertar, ele limpa a quest e recupera seu XP parcial acumulado (a menos que use um artefato purificador ou multiplicador como a Poção de Cura ou o Elixir Dourado).
...
*   Para evitar que um aluno cometa um erro, saia da Party e entre em outra para resetar suas chances, o timestamp do cooldown é gravado de forma persistente em seu perfil (`partyCooldownUntil`). Ele fica bloqueado de responder a essa quest (em grupo ou solo) até que o tempo do cooldown expire.

---

## 🐉 5. Mini Bosses, Mega Boss Raids e Efeitos Visuais/Sonoros

Os combates de Boss oferecem grandes quantidades de XP, combate cooperativo e drop de artefatos mágicos:

### A. Ativação de Mini Bosses (Triggers D&D)
...
*   *Fórmula*: `600 / quantidade_de_bosses`. Sorteados a partir de uma pool de 50 monstros de D&D.

### B. Mega Boss Raids de Turma (Evento do Mestre)
*   **Exclusividade de Party**: O combate ao Mega Boss exige obrigatoriamente a participação em uma **Party ativa**. Alunos em modo solo são instruídos a ingressar ou criar uma Party.
*   **Backlog Coletivo de Quests & Trava de 15 Minutos**: As quests do Boss são organizadas em um Mural Público. Uma Party reserva uma quest para resolução. Se a Party falhar ou ficar inativa por 15 minutos, a quest retorna ao backlog acumulando bônus de drop de artefatos (`💀 Dizimou X Parties` — +50% bônus de drop por derrota acumulada).
*   **Catálogo D&D & Sorteador por HP**: O Mestre pode escolher diretamente o monstro (Yuan-Ti Abominação, Dragão Vermelho, Beholder, Lich, Mind Flayer, Tarrasque, Demogorgon, etc.) ou usar o botão de sorteio automático proporcional ao HP do evento.
*   **Gerenciador & Transmutação de Quests pelo Mestre**: O Mestre visualiza o pool ativo de questões do Boss, podendo **editar enunciados manualmente** ou acionar a **Transmutação por IA (Gemini)** para recalibrar perguntas individualmente.

### C. Modos de Resposta Calibrados por Disciplina
*   **`FOTO_REDACAO`**: Restrito exclusivamente a disciplinas de **Redação** (escaneamento OCR de folha escrita à mão).
*   **`FOTO_MATEMATICA`**: Restrito a disciplinas de exatas (**Matemática, Física e Química**).
*   **`TEXTO`**: Resposta dissertativa em texto livre usada por padrão para **Português**, História, Geografia e disciplinas gerais.

### D. Experiência Visual, Áudio & Modal Responsivo
*   **Música de Tensão**: Abertura do modal de Boss aciona a trilha sonora épica `boss_arena.mp3` em loop, interrompida no fechamento.
*   **Fogo Retro DoomFire**: Fundo com animação de partículas de fogo retro (`DoomFireParticles`).
*   **Modal Expandido em 60% da Tela**: Abertura da Carta Manhwa do Monstro em tela cheia ocupando **60% da viewport** (`60vw` / `60vh`) com `objectPosition: center top` para enquadramento sem cortes do rosto e chifres dos monstros na Web e Mobile.
*   **SystemAlert Cybernético**: Todos os alertas do sistema utilizam o modal escuro neon padronizado ([SystemAlert.tsx](file:///home/beni/Documentos/Projetos/solo-learning/frontend/components/SystemAlert.tsx)).
*   **Shaders de Queima (Skia)**: Animação física GPU (`CardBurnEffect` via Skia e fragment shaders) exibindo combustão incandescente ao consumir artefatos.
...
O resolvedor automático de grades horárias aplica restrições baseadas na legislação de ensino brasileira:
...
onarch Engine | Quests & Gamificação | Ordinator (IA) |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Trial** (14 dias) | Gratuito | - | Limitado (2 turmas) | Ativo | Bloqueado |
| **Rank B** (Gamificação) | R$ 2,50 | R$ 22,00 | Bloqueado | Ativo | Bloqueado |
| **Rank A** (Premium) | R$ 3,50 | R$ 30,00 | Ilimitado | Ativo | Bloqueado |
