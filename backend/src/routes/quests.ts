...
import { NvidiaAIProvider } from '../infra/providers/nvidia/NvidiaAIProvider';
...
export const questsRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  const aiProvider = new NvidiaAIProvider();
  ...
  const callGemini = async (prompt: string, image?: { data: string, mimeType: string }, attempt: number = 0): Promise<string> => {
    ...
    // 1. TENTAR NVIDIA PRIMEIRO (apenas se não tiver imagem, pois o modelo Llama 3.1 não suporta visão neste endpoint)
    if (process.env.NVIDIA_API_KEY && !image) {
      try {
        const nvidiaRes = await globalThis.fetch('https://integrate.api.nvidia.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              { role: 'system', content: cavemanSystemPrompt },
        ...
        console.warn('[Quests] Falha na Nvidia, tentando fallback para Gemini...');
      }
    }
...
Seja inteligente e flexível na correspondência de letras e textos!`;
  ...
  fastify.post<{ Body: { professorId: string; disciplinaId: string; temp?: any; turmaIds?: string[]; aulasSemanais?: number } }>('/disciplinas/professor', { preValidation: [fastify.authenticate] }, async (request, reply) => {
    try {
      ...
      if (!parsedTemp) {
        const existingStandard = await prisma.disciplinaProfessor.findFirst({
          where: { professorId, temp: false, disciplinaId: { not: disciplinaId } }
        });
        if (existingStandard) {
          const existingDisc = await prisma.disciplina.findUnique({ where: { id: existingStandard.disciplinaId } });
          return reply.status(400).send({ error: `Este professor já possui uma matéria padrão (${existingDisc?.nome}). Um professor só pode ter uma matéria padrão, as demais devem ser marcadas como substituto (temp).` });
        }
      }
